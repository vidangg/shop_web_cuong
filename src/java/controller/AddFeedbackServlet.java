...
public class AddFeedbackServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // Tạo đối tượng Feedback từ dữ liệu lấy được
        Feedback newFeedback = new Feedback(accountId, bookId, feedbackText);

        // Gọi DAO để thêm phản hồi vào CSDL
        FeedbackDAO feedbackDAO = new FeedbackDAO();
        boolean success = feedbackDAO.addFeedback(newFeedback);

        if (success) {
            // Nếu thành công, chuyển hướng lại trang updateFeedback.jsp hoặc trang chi tiết sản phẩm
            response.sendRedirect("updateFeedback.jsp?bid=" + bookId);
        } else {
            // Xử lý lỗi (nếu có)
